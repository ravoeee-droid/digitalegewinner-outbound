...
export async function POST(request:Request){
 try{
  ...
  const auditPoints=(audit?.sales?.loomTalkingPoints||audit?.priorities?.slice(0,3).map(p=>`${p.title||"Hebel"}: ${p.action||"prüfen"}`)||[]) as string[];
  const talkingPoints=auditPoints.length?auditPoints:notes?[notes]:["Aktuelle Recruiting-Situation kurz einordnen","Konkreten Bewerber-Hebel zeigen","Einfachen nächsten Schritt anbieten"];
  const opener=String(audit?.sales?.opener||notes||"Ich habe mir Ihren aktuellen Recruiting-Einstieg kurz angesehen und einen konkreten Hebel gefunden.");
  const payload={jobId:crypto.randomUUID(),leadId,company:String(lead.company||""),contact:String(lead.contact||""),website:String(lead.website||""),city:String(lead.city||""),industry:String(lead.industry||""),websiteScore:Number(lead.websiteScore||audit?.scores?.overall||0),intentScore:Number(lead.intentScore||0),opener,talkingPoints,analysisUrl:`${base}/a/${encodeURIComponent(leadId)}`,callbackUrl,brand:{name:"Digitale Gewinner",sender:String(state?.settings?.senderName||"Raphael Hermann")}};
