...
function fromHeader(value:string){const match=value.match(/<([^>]+)>/);return (match?.[1]||value).trim().toLowerCase()}

async function recordReply(messageId:string,from:string,subject:string,mailboxId:string){
  const existing=await query<{id:number}>("select id from er_events where workspace=$1 and type='reply' and meta->>'providerMessageId'=$2 limit 1",[WORKSPACE,messageId]);
  ...
  await query("insert into er_events(workspace,lead_id,type,meta) values($1,$2,'reply',$3::jsonb)",[WORKSPACE,leadId,JSON.stringify({providerMessageId:messageId,from,subject,mailboxId})]);
  await query("update er_outbox set status='stopped' where workspace=$1 and lead_id=$2 and status='queued'",[WORKSPACE,leadId]);
  if(state?.leads){
    const leads=state.leads.map((item)=>String(item.id)===leadId?{...item,intentScore:Math.min(100,Number(item.intentScore||0)+30),stage:["Neu","Kontaktiert"].includes(String(item.stage||""))?"Engaged":item.stage}:item);
...
async function syncGmail(credential:Credential){
  ...
  for(const message of data.messages||[]){
    const response=await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${message.id}?format=metadata&metadataHeaders=From&metadataHeaders=Subject`,{headers:{Authorization:`Bearer ${token}`}});
    if(!response.ok)continue;
    const item=await response.json() as {id:string;payload?:{headers?:Array<{name:string;value:string}>}};
    ...
    if(await recordReply(item.id,from,subject,credential.id))replies+=1;
  }
  return replies;
}
...
async function syncMicrosoft(credential:Credential){
  ...
  for(const message of data.value||[]){
    ...
    if(!from||from===credential.email.toLowerCase())continue;
    if(await recordReply(message.id,from,message.subject||"",credential.id))replies+=1;
  }
  return replies;
}
...
async function run(request:Request){
  ...
  for(const credential of credentials){
    ...
    }catch(error){errors.push(`${credential.id}: ${error instanceof Error?error.message:"Sync Fehler"}`)}
  }
