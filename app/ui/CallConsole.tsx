...
type Outcome = "not_reached" | "callback" | "pain" | "appointment" | "no_fit" | "dnc" | "website_requested";
    ...
    <main className="rapid-root" aria-busy={busy}>
      ...
      {!active ? (
        ...
        <section className="rapid-card" aria-label={`Aktiver Lead: ${company}`}>
          <div className="rapid-topline"><span>Lead #{active.rank}</span><span>Score {active.score} von 100</span></div>
          <div className="rapid-main">
            <section className="rapid-company" aria-labelledby="company-name">
              <p>NÄCHSTER LEAD</p><h1 id="company-name">{company}</h1>
              <div className="rapid-meta">{city && <span>{city}</span>}{contact && <span>{contact}</span>}</div>
              <button className="rapid-phone" onClick={dial} disabled={!phone || busy} aria-label={`${company} unter ${phone || "unbekannter Nummer"} anrufen`}>
                <small>ENTER · ANRUFEN</small><strong>{phone || "Keine Nummer"}</strong>
              </button>
