...
type Overview = {
  ...
  leads: Lead[];
  activities: Activity[];
  behaviorTriggerSync?: { scanned: number; processed: number; refreshedActions: number };
};
...
const actionLabels: Record<string, string> = {
  reply_now: "Antworten – jetzt",
  call_now: "Anrufen – jetzt",
  prepare_call: "Termin vorbereiten",
  prepare_offer: "Angebot vorbereiten",
  personal_followup: "Persönlich nachfassen",
  followup_after_video: "Nach Video nachfassen",
  followup_after_view: "Nach Seitenbesuch nachfassen",
  send_personalized_loom: "Personalisiertes Loom senden",
  call_first: "Lead zuerst anrufen",
  research_more: "Research vertiefen",
};
...
function actionBadge(action = "") {
  const hot = ["reply_now", "call_now", "prepare_call", "prepare_offer"].includes(action);
  return {
    background: hot ? "#2e1710" : "#10241d",
    color: hot ? "#ffb37c" : "#7cf0b8",
    border: `1px solid ${hot ? "#8c4e2c" : "#335f4e"}`,
    borderRadius: 10,
    padding: "7px 9px",
    fontSize: 10,
    fontWeight: 900,
...
export default function SalesOSWidget() {
  ...
  async function scanTriggers() {
    ...
    setError("");
    try {
      const response = await fetch("/api/triggers/scan", {
        method: "POST",
      ...
      const json = await response.json() as {
        error?: string;
        result?: { due?: number; results?: Array<{ created?: number; baseline?: boolean; company?: string; error?: string }> };
        behaviorTriggerSync?: { processed?: number; refreshedActions?: number };
      };
      ...
      const behavior = json.behaviorTriggerSync?.processed || 0;
      setMessage(`${json.result?.due || 0} Firmen geprüft · ${signals} neue Firmensignale · ${behavior} neue Verhaltenssignale${baselines ? ` · ${baselines} Baseline` : ""}`);
      await load();
    } catch (e) {
  ...
  return <>
    ...
    {open && <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(3,6,10,.82)", backdropFilter: "blur(9px)", display: "grid", placeItems: "center", padding: 20 }}>
      <div onClick={(event) => event.stopPropagation()} style={{ width: "min(1220px,100%)", maxHeight: "90vh", overflow: "auto", background: "#09111a", border: "1px solid #26384b", borderRadius: 24, padding: 24, color: "#eef6ff", fontFamily: "Inter,system-ui" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 18 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 900, letterSpacing: ".16em", color: "#61e6a5" }}>DIGITALE GEWINNER · SALES OPERATING SYSTEM</div>
            <h2 style={{ margin: "8px 0 5px", fontSize: 30 }}>Trigger Intelligence + Next Best Action</h2>
            <p style={{ margin: 0, color: "#8494a8", fontSize: 13 }}>Vor dem Outreach erkennt der Radar Firmenveränderungen. Nach dem Outreach erkennt die Intent Engine Views, Video-Interesse, Klicks und Antworten – und setzt die nächste Sales-Aktion.</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 10, marginTop: 10 }}>
          {[["Firmen-Monitore", triggerOverview.stats.monitors], ["Jetzt fällig", triggerOverview.stats.due], ["Firmensignale 24h", triggerOverview.stats.signals_24h], ["Starke Firmensignale", triggerOverview.stats.hot_signals]].map(([label, value]) => <div key={String(label)} style={{ border: "1px solid #234232", background: "#0c1d17", borderRadius: 14, padding: 13 }}><small style={{ color: "#6f9c87", fontWeight: 800 }}>{label}</small><strong style={{ display: "block", marginTop: 5, fontSize: 22, color: "#7cf0b8" }}>{value}</strong></div>)}
        </div>

        <div style={{ marginTop: 9, color: "#62788e", fontSize: 10 }}>
          Verhaltens-Engine: {overview.behaviorTriggerSync?.processed || 0} neue Signale verarbeitet · {overview.behaviorTriggerSync?.refreshedActions || 0} Next Actions neu bewertet
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1.5fr .9fr", gap: 14, marginTop: 16 }}>
          <section style={{ border: "1px solid #1d2d3c", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "13px 15px", borderBottom: "1px solid #1d2d3c", fontWeight: 900 }}>🔥 Priority + Next Action Queue</div>
            <div style={{ display: "grid" }}>
              ...
              {overview.leads.map((lead) => <div key={lead.id} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 14, padding: 14, borderBottom: "1px solid #142332" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <strong style={{ fontSize: 13 }}>{lead.company}</strong>
                    {lead.next_action && <span style={actionBadge(lead.next_action)}>⚡ {actionLabels[lead.next_action] || lead.next_action}</span>}
                  </div>
                  <div style={{ marginTop: 5, fontSize: 11, color: "#768aa0" }}>{[lead.city, lead.industry, lead.email || lead.phone].filter(Boolean).join(" · ")}</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 9 }}>
                    <span style={{ ...badge(lead.opportunity_score), fontSize: 10 }}>Opportunity {lead.opportunity_score}</span>
                    <span style={{ ...badge(lead.fit_score), fontSize: 10 }}>Fit {lead.fit_score}</span>
                    <span style={{ ...badge(lead.intent_score), fontSize: 10 }}>Intent {lead.intent_score}</span>
                    <span style={{ ...badge(lead.trigger_score || 0), fontSize: 10 }}>Behavior {lead.trigger_score || 0}</span>
                  </div>
                  {lead.next_action_reason && <div style={{ marginTop: 9, color: "#9eb0c2", fontSize: 11, lineHeight: 1.45 }}>{lead.next_action_reason}</div>}
                </div>
                <div style={{ textAlign: "right", minWidth: 112 }}>
                  <span style={badge(lead.priority_score)}>Priority {lead.priority_score}</span>
                  <div style={{ marginTop: 9, fontSize: 10, color: "#718296" }}>{lead.stage}</div>
                  {!!lead.next_action_priority && <div style={{ marginTop: 5, fontSize: 10, color: "#61e6a5" }}>Action {lead.next_action_priority}/100</div>}
                </div>
              </div>)}
            </div>
          </section>
          ...
          <section style={{ border: "1px solid #28513e", borderRadius: 16, overflow: "hidden", background: "#0a1511" }}>
            <div style={{ padding: "13px 15px", borderBottom: "1px solid #234232", fontWeight: 900, color: "#81f4bd" }}>⚡ Company Trigger Radar</div>
            <div>
              {triggerOverview.triggers.length === 0 && <div style={{ padding: 18, color: "#718b7e", fontSize: 12 }}>Noch keine Firmensignale. Der erste Scan erstellt pro Firma eine Baseline; danach werden relevante Änderungen erkannt.</div>}
              {triggerOverview.triggers.map((trigger) => <div key={trigger.id} style={{ padding: 13, borderBottom: "1px solid #193326" }}>
        ...
        <section style={{ border: "1px solid #1d2d3c", borderRadius: 16, overflow: "hidden", marginTop: 14 }}>
          <div style={{ padding: "13px 15px", borderBottom: "1px solid #1d2d3c", fontWeight: 900 }}>Live Activity Stream</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))" }}>
            {overview.activities.length === 0 && <div style={{ padding: 18, color: "#74869a", fontSize: 12 }}>Noch keine Activities.</div>}
            {overview.activities.map((activity) => <div key={activity.id} style={{ padding: 13, borderBottom: "1px solid #142332", borderRight: "1px solid #142332" }}><strong style={{ display: "block", fontSize: 11 }}>{activity.summary}</strong><small style={{ display: "block", marginTop: 5, color: "#687c91" }}>{new Date(activity.created_at).toLocaleString("de-DE")}</small></div>)}
          </div>
        </section>
      </div>
    </div>}
