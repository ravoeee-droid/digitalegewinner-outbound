...
export default function SalesOSWidget() {
  ...
  return <>
    <button onClick={() => { setOpen(true); void load(); }} style={{ position: "fixed", right: 24, bottom: 78, zIndex: 80, border: "1px solid #335f4e", background: "#10241d", color: "#7cf0b8", padding: "12px 15px", borderRadius: 13, fontWeight: 900, cursor: "pointer", boxShadow: "0 18px 45px rgba(0,0,0,.35)" }}>⚡ Pflege Sales OS</button>
    {open && <div onClick={() => setOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(3,6,10,.82)", backdropFilter: "blur(9px)", display: "grid", placeItems: "center", padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "min(1100px,100%)", maxHeight: "90vh", overflow: "auto", background: "#09111a", border: "1px solid #26384b", borderRadius: 24, padding: 24, color: "#eef6ff", fontFamily: "Inter,system-ui" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 18 }}>
          <div><div style={{ fontSize: 10, fontWeight: 900, letterSpacing: ".16em", color: "#61e6a5" }}>PFLEGE RECRUITING · SALES OPERATING SYSTEM</div><h2 style={{ margin: "8px 0 5px", fontSize: 30 }}>CRM + Recruiting Intelligence</h2><p style={{ margin: 0, color: "#8494a8", fontSize: 13 }}>Normalisierte Pflege-Unternehmen, Kontakte, Leads, Research Runs und Aktivitäten. Priority kombiniert Fit, Opportunity, Kontaktierbarkeit und Intent.</p></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.55fr .85fr", gap: 14, marginTop: 16 }}>
          <section style={{ border: "1px solid #1d2d3c", borderRadius: 16, overflow: "hidden" }}>
            <div style={{ padding: "13px 15px", borderBottom: "1px solid #1d2d3c", fontWeight: 900 }}>🔥 Pflege Priority Queue</div>
            <div style={{ display: "grid" }}>
              {overview.leads.length === 0 && <div style={{ padding: 20, color: "#74869a", fontSize: 13 }}>Noch keine normalisierten Pflege-Leads. Importiere im Pflege Lead Finder den ersten Lead.</div>}
          ...
          <section style={{ border: "1px solid #1d2d3c", borderRadius: 16, overflow: "hidden" }}>
            ...
            <div>{overview.activities.length === 0 && <div style={{ padding: 18, color: "#74869a", fontSize: 12 }}>Noch keine Aktivitäten.</div>}{overview.activities.map(activity => <div key={activity.id} style={{ padding: 13, borderBottom: "1px solid #142332" }}><strong style={{ display: "block", fontSize: 11 }}>{activity.summary}</strong><small style={{ display: "block", marginTop: 5, color: "#687c91" }}>{new Date(activity.created_at).toLocaleString("de-DE")}</small></div>)}</div>
          </section>
        </div>
      </div>
    </div>}
