...
export default function PflegeProOS() {
  ...
  function ActivityList({ items }: { items: Activity[] }) {
    return <div className={styles.activityList}>{items.map((item) => <div className={styles.activity} key={item.id}><span>{item.type.includes("call") ? "◉" : item.type.includes("enrichment") ? "◈" : item.type.includes("studio") ? "▶" : "·"}</span><div><strong>{item.summary}</strong><small>{fmtDate(item.created_at, true)}</small></div></div>)}{!items.length && <div className={styles.empty}>Noch keine Aktivitäten.</div>}</div>;
  }
  ...
  return <div className={styles.root}>
    ...
    <div className={styles.shell}>{nav}<section className={styles.workspace}>
      ...
      <main className={styles.content} id="main-sales-os">
        ...
        {view === "campaigns" && <><div className={styles.pageHead}><div><span>CAMPAIGNS</span><h2>Outbound-Sequenzen.</h2><p>Mailboxen, Versandlimit, Replies und Meetings ohne übergroße Cards.</p></div><span className={styles.softBadge}>{activeMailboxes.length} active mailboxes</span></div><section className={styles.panel}><div className={styles.tableWrap}><table className={styles.campaignTable}><thead><tr><th>Campaign</th><th>Audience</th><th>Status</th><th>Sequence</th><th>Sent</th><th>Replies</th><th>Positive</th><th>Meetings</th><th></th></tr></thead><tbody>{store.campaigns.map((campaign) => <tr key={campaign.id}><td><strong>{campaign.name}</strong></td><td>{campaign.audience}</td><td><span className={styles.softBadge}>{campaign.status}</span></td><td>{campaign.steps.length} steps · {campaign.dailyLimit}/d</td><td>{campaign.sent || 0}</td><td>{campaign.replies || 0}</td><td>{campaign.positive || 0}</td><td>{campaign.appointments || 0}</td><td><button className={styles.primaryTiny} type="button" onClick={() => void launchCampaign(campaign)} disabled={busy === campaign.id || campaign.status === "Aktiv"}>{campaign.status === "Aktiv" ? "Running" : "Launch"}</button></td></tr>)}</tbody></table></div></section></>}
