...
function validForCall(lead: Lead) { return Boolean(phoneOf(lead)) && !lead.do_not_contact && lead.phone_status !== "invalid" && !["Gewonnen", "Verloren"].includes(lead.stage); }
...
function surfaceCandidateScore(lead: FoundLead) {
  let score = 40;
  if (lead.phone) score += 25;
  if (lead.website) score += 20;
  if (/pflege|ambulant|intensiv|senior/i.test(`${lead.company} ${lead.industry}`)) score += 15;
  return clamp(score);
}
...
export default function PflegeCloserOS() {
  ...
  async function searchQuery(query: string, pageSize = 8) {
    const response = await fetch("/api/leads/search", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ query, pageSize }) });
    const json = await response.json() as { leads?: FoundLead[]; error?: string };
    if (!response.ok) throw new Error(json.error || "Lead-Suche fehlgeschlagen.");
    return json.leads || [];
  }
  async function importCandidate(candidate: FoundLead, silent = false) {
    setBusy(`import-${candidate.id}`);
    try {
      const response = await fetch("/api/radar/import", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...candidate, source: "silver-platter", workspace: "default" }) });
      const json = await response.json() as { leadId?: string; error?: string };
      if (!response.ok) throw new Error(json.error || "Import fehlgeschlagen.");
      if (json.leadId) {
        await fetch("/api/crm/enrichment", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ leadIds: [json.leadId], ai: true }) });
      }
      await loadCrm();
      if (!silent) notify(`${candidate.company} recherchiert und ins CRM gelegt.`);
    } catch (e) { if (!silent) notify(e instanceof Error ? e.message : "Import fehlgeschlagen."); else throw e; }
    finally { setBusy(""); }
  }
  async function buildSilverPlatter(event?: FormEvent<HTMLFormElement>) {
    event?.preventDefault();
    if (factory.running) return;
    const queries = [
      `Ambulanter Pflegedienst ${region}`,
      `Pflegedienst ${region}`,
      `Intensivpflegedienst ${region}`,
      `Ambulante Pflege ${region}`,
    ];
    setFactory({ running: true, current: 0, total: 8, message: "Suche passende Pflegebetriebe …" });
    setError("");
    try {
      const batches = await Promise.all(queries.map((query) => searchQuery(query, 8)));
      const existing = new Set(data.leads.map((lead) => lead.company.trim().toLowerCase()));
      const unique = new Map<string, FoundLead>();
      for (const item of batches.flat()) {
        const key = `${item.company.trim().toLowerCase()}|${item.phone.replace(/\D/g, "")}`;
        if (!existing.has(item.company.trim().toLowerCase()) && !unique.has(key)) unique.set(key, item);
      }
      const ranked = [...unique.values()].sort((a, b) => surfaceCandidateScore(b) - surfaceCandidateScore(a));
      setCandidates(ranked.slice(0, 16));
      const top = ranked.slice(0, 8);
      setFactory({ running: true, current: 0, total: top.length, message: `${top.length} Kandidaten gefunden · Research läuft …` });
      let completed = 0;
      for (let index = 0; index < top.length; index += 2) {
        const slice = top.slice(index, index + 2);
        await Promise.all(slice.map((candidate) => importCandidate(candidate, true).catch(() => undefined)));
        completed += slice.length;
        setFactory({ running: true, current: completed, total: top.length, message: `${completed}/${top.length} Leads geprüft und einsortiert` });
      }
      await loadCrm();
      setFactory({ running: false, current: top.length, total: top.length, message: `${top.length} neue Leads recherchiert · die besten liegen oben auf dem Silbertablett.` });
      setView("focus");
      notify("Silver Platter aktualisiert.");
    } catch (e) {
      const message = e instanceof Error ? e.message : "Lead Factory fehlgeschlagen.";
      setFactory((current) => ({ ...current, running: false, message }));
      setError(message);
    }
  }
  ...
  return <div className={styles.root}><div className={styles.shell}>
    <header className={styles.topbar}>
      ...
      <div className={styles.topActions}><a className={styles.advancedLink} href="/pro">Pro Workspace ↗</a><button className={styles.quiet} type="button" onClick={() => void refresh()} disabled={busy === "refresh"}>Aktualisieren</button><button className={styles.primary} type="button" onClick={() => setView("factory")}>+ Top-Leads holen</button></div>
    </header>

    {error && <div className={styles.error}>{error}</div>}

    <section className={styles.mission}>
      <div className={styles.hero}><span className={styles.eyebrow}>HEUTIGE MISSION</span><h1>30 Entscheider. 5 Termine. 1 Abschluss.</h1><p>Kein Suchen, kein Überlegen: Das System sortiert dir die stärksten Pflege-Leads nach Abschlusschance und zeigt dir beim Call exakt Pain, Pitch, Einwand und nächste Aktion.</p><div className={styles.heroActions}><button className={styles.success} type="button" onClick={() => { setView("focus"); if (selected) dial(selected); }} disabled={!selected}>▶ Nächsten Top-Lead anrufen</button><button className={styles.quiet} type="button" onClick={() => setView("factory")}>8 neue Leads finden + Research</button></div></div>
      <div className={styles.missionStats}>
        ...
        <div className={styles.stat}><span>Pipeline</span><strong>{euro(data.stats.pipeline)}</strong><small>{dueNow} Follow-ups fällig</small><div className={styles.progress}><i style={{ width: `${Math.min(100, data.stats.hot * 8)}%` }} /></div></div>
      </div>
    </section>

    <div className={styles.tabs}><button className={view === "focus" ? styles.tabActive : ""} onClick={() => setView("focus")}>Silbertablett</button><button className={view === "factory" ? styles.tabActive : ""} onClick={() => setView("factory")}>Lead Factory</button><button className={view === "all" ? styles.tabActive : ""} onClick={() => setView("all")}>Alle Leads</button></div>

    {view === "focus" && <div className={styles.mainGrid}>
      <section className={styles.panel}><div className={styles.panelHead}><div><span className={styles.eyebrow}>SILVER PLATTER</span><h2>Die Leads, die du jetzt anrufen sollst.</h2><p>Score = Fit + Opportunity + Intent + Recruiting-Pain + Erreichbarkeit + Follow-up-Druck.</p></div><span className={`${styles.badge} ${perfect.length ? styles.badgeHot : ""}`}>{perfect.length} Top-Chancen</span></div>
        <div className={styles.leadList}>{platter.slice(0, 24).map((lead) => { const score = silverScore(lead); const pains = painSignals(lead); return <button type="button" key={lead.id} className={`${styles.leadRow} ${selected?.id === lead.id ? styles.leadRowActive : ""}`} onClick={() => { setSelectedId(lead.id); setObjection(""); }}><div className={`${styles.score} ${score >= 78 ? styles.scoreHot : ""}`}>{score}</div><div className={styles.company}><strong>{lead.company}</strong><small>{[lead.contact || "PDL offen", lead.city].filter(Boolean).join(" · ")}</small></div><div className={styles.pain}><strong>{pains[0] || primaryPain(lead)}</strong><small>{pains.slice(1,3).join(" · ") || `Fit ${lead.fit_score} · Opportunity ${lead.opportunity_score}`}</small></div><div className={styles.miniMeta}><strong>{lead.stage}</strong><small>{phoneOf(lead) ? "● call-ready" : "kein Telefon"}</small></div></button>; })}{!platter.length && <div className={styles.empty}>Noch keine call-ready Leads. Öffne die Lead Factory und lass dir die ersten Top-Leads bauen.</div>}</div>
      </section>
      ...
      <aside className={styles.callPanel}>{selected ? <>
        ...
        <div className={styles.painBox}><label>PAIN RADAR · DARÜBER REDEN</label><div className={styles.painTags}>{painSignals(selected).map((pain) => <span key={pain}>{pain}</span>)}{!painSignals(selected).length && <span>Recruiting-Prozess im Gespräch qualifizieren</span>}</div></div>
    ...
    {view === "factory" && <section className={styles.factoryPanel}><div className={styles.panelHead}><div><span className={styles.eyebrow}>LEAD FACTORY</span><h2>Neue Pflege-Leads automatisch suchen, prüfen und vorsortieren.</h2><p>Google Places → Website/Contact Research → Recruiting-Pain → CRM → Silver Score.</p></div><span className={styles.badge}>kein CSV-Gefummel</span></div><form className={styles.factoryTop} onSubmit={(event) => void buildSilverPlatter(event)}><div className={styles.field}><label>REGION / GEBIET</label><input value={region} onChange={(e) => setRegion(e.target.value)} placeholder="z. B. Stuttgart, Baden-Württemberg" /></div><button className={styles.success} disabled={factory.running}>{factory.running ? "Research läuft …" : "8 Top-Leads finden + recherchieren"}</button></form>{factory.message && <div className={styles.factoryProgress}>{factory.message}{factory.total > 0 && <div className={styles.progress}><i style={{ width: `${Math.round(factory.current / factory.total * 100)}%` }} /></div>}</div>}<div className={styles.factoryGrid}>{candidates.map((candidate) => <article className={styles.candidate} key={candidate.id}><strong>{candidate.company}</strong><p>{candidate.city}</p><div className={styles.candidateFooter}><span>Pre-Score {surfaceCandidateScore(candidate)} · {candidate.phone ? "Telefon" : "kein Telefon"} · {candidate.website ? "Website" : "keine Website"}</span><button type="button" onClick={() => void importCandidate(candidate)} disabled={busy === `import-${candidate.id}`}>Import + Research</button></div></article>)}</div>{!candidates.length && !factory.running && <div className={styles.empty}>Region eingeben und Lead Factory starten. Die stärksten Treffer werden automatisch recherchiert und danach im Silbertablett einsortiert.</div>}</section>}

    {view === "all" && <section className={styles.panel}><div className={styles.panelHead}><div><span className={styles.eyebrow}>CRM QUICK VIEW</span><h2>Alle Pflege-Leads.</h2></div><span className={styles.badge}>{data.leads.length} Leads</span></div><div className={styles.tableWrap}><table className={styles.allTable}><thead><tr><th>Score</th><th>Unternehmen</th><th>Ort</th><th>Stage</th><th>Fit</th><th>Opportunity</th><th>Next</th><th>Aktion</th></tr></thead><tbody>{[...data.leads].sort((a,b) => silverScore(b)-silverScore(a)).map((lead) => <tr key={lead.id}><td><b>{silverScore(lead)}</b></td><td><b>{lead.company}</b></td><td>{lead.city}</td><td>{lead.stage}</td><td>{lead.fit_score}</td><td>{lead.opportunity_score}</td><td>{lead.next_action || "—"}</td><td><button type="button" onClick={() => { setSelectedId(lead.id); setView("focus"); }}>Öffnen</button></td></tr>)}</tbody></table></div></section>}
