...
function extract(html:string,base:URL){
 ...
 let linkedin="",instagram="";for(const match of decoded.matchAll(/href\s*=\s*["']([^"']+)["']/gi)){const href=match[1];if(!linkedin&&/linkedin\.com\//i.test(href))linkedin=new URL(href,base).toString();if(!instagram&&/instagram\.com\//i.test(href))instagram=new URL(href,base).toString()}
 const pageLinks:string[]=[];for(const match of decoded.matchAll(/<a\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){const href=match[1];const label=match[2].replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();if(/kontakt|contact|impressum|imprint|about|über uns|unternehmen/i.test(`${href} ${label}`)){try{const candidate=new URL(href,base);if(candidate.hostname===base.hostname&&["http:","https:"].includes(candidate.protocol))pageLinks.push(candidate.toString())}catch{}}}
 return{emails,phones,linkedin,instagram,pageLinks:unique(pageLinks)};
}
async function getHtml(url:URL){await safeUrl(url);const response=await fetch(url,{redirect:"follow",cache:"no-store",headers:{"user-agent":"DigitaleGewinner-ContactEnrichment/1.0",accept:"text/html,application/xhtml+xml"},signal:AbortSignal.timeout(9000)});if(!response.ok)throw new Error(`Website HTTP ${response.status}`);const type=response.headers.get("content-type")||"";if(!type.includes("text/html")&&!type.includes("application/xhtml+xml"))throw new Error("Keine HTML-Seite.");const declared=Number(response.headers.get("content-length")||0);if(declared>MAX_BYTES)throw new Error("Website zu groß.");const bytes=await response.arrayBuffer();if(bytes.byteLength>MAX_BYTES)throw new Error("Website zu groß.");return{html:new TextDecoder().decode(bytes),url:new URL(response.url||url.toString())}}

export async function enrichPublicContact(rawUrl:string):Promise<ContactEnrichment>{
 const start=normalize(rawUrl);const queue=[start.toString()];const visited=new Set<string>();let emails:string[]=[],phones:string[]=[],linkedin="",instagram="",contactPage="";
 while(queue.length&&visited.size<MAX_PAGES){const raw=queue.shift()!;if(visited.has(raw))continue;visited.add(raw);try{const page=await getHtml(new URL(raw));if(page.url.hostname!==start.hostname)continue;const found=extract(page.html,page.url);emails=unique([...emails,...found.emails]);phones=unique([...phones,...found.phones]);linkedin=linkedin||found.linkedin;instagram=instagram||found.instagram;if(raw!==start.toString()&&!contactPage)contactPage=raw;for(const link of found.pageLinks){if(!visited.has(link)&&queue.length+visited.size<MAX_PAGES+2)queue.push(link)}}catch{}}
